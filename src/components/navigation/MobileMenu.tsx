...
export function MobileMenu({
  ...
  return (
    <>
      <div
        className={cn(
          "fixed inset-0 z-50 flex items-center justify-center bg-background/95 backdrop-blur-md transition-opacity duration-300",
      ...
      >
        <div
          className={cn(
            "relative flex flex-col items-center gap-12 p-8 transition-all duration-300",
        ...
        >
          <button
            type="button"
            onClick={onClose}
            className="absolute top-6 right-6 text-foreground-muted hover:text-foreground"
            aria-label="Close menu"
          >
            <span className="block h-0.5 w-6 bg-foreground rotate-45 translate-y-0" />
            <span className="mt-1 block h-0.5 w-6 bg-foreground -rotate-45" />
          </button>
          ...
          <nav className="flex flex-col gap-1">
            {navLinks.map((link) => (
              <Link
                ...
                onClick={onClose}
                className={cn(
                  "px-8 py-4 text-center text-2xl font-display font-bold tracking-widest uppercase transition-all duration-200 hover:text-accent-red hover:translate-x-2",
                  isActive(link.href)
                    ? "text-accent-red"
                    : "text-foreground"
                )}
...
function SocialLink({
  ...
  return (
    <a
      href={href}
      aria-label={label}
      className="flex h-10 w-10 items-center justify-center rounded-full border border-foreground-muted text-foreground-muted transition-all duration-200 hover:border-accent-red hover:text-accent-red"
    >
      {children}
    </a>
  );
...
function FacebookIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
      <path d="M22.675 0h-21.35C.6 0 0 .6 0 1.625v20.75C0 23.4.6 24 1.625 24h11.34v-9.294H9.85v-3.574h3.115V8.414c0-3.1 1.893-4.788 4.658-4.788 1.325 0 2.466.099 2.796.143v3.24l-1.918.001c-1.503 0-1.795.715-1.795 1.764v2.312h3.587l-.467 3.576h-3.11V24h6.1c1.025 0 1.625-.6 1.625-1.625V1.625C24 .6 23.4 0 22.675 0" />
    </svg>
  );
}

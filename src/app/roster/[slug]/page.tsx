...
export default async function WrestlerProfile({ params }: PageProps) {
  ...
  const winRateLabel =
    recordTotal > 0
      ? `${calculateWinRate(wrestler.record)}%`
      : "N/A";
  ...
  return (
    <>
      ...
      <section className="profile-enter relative overflow-hidden border-b border-border bg-background">
        {/* Background image */}
        ...
        >
          {(wrestler.imageFull || wrestler.image) && (
            <Image
              ...
              priority
              sizes="100vw"
              className="object-cover object-top opacity-[0.10] grayscale"
            />
          )}
          ...
          <div className="profile-glow absolute -right-40 top-1/4 h-96 w-96 rounded-full bg-accent-red/10 blur-3xl" />

          <div className="absolute -left-32 bottom-0 h-72 w-72 rounded-full bg-accent-gold/[0.06] blur-3xl" />
        </div>

        <div className="container-wide relative z-10 py-10 sm:py-14 md:py-20 lg:py-24">
          <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_340px] xl:grid-cols-[minmax(0,1fr)_390px] xl:gap-16">
            {/* =====================================================
                TEXT
            ===================================================== */}
            ...
            <div className="min-w-0 lg:order-1">
              ...
              <Link
                href="/roster"
                className="group inline-flex items-center gap-2 font-body text-[10px] font-bold tracking-[0.22em] text-foreground-muted transition-colors duration-200 hover:text-accent-red sm:text-xs"
              >
                <span className="grid h-7 w-7 place-items-center border border-border transition-colors duration-200 group-hover:border-accent-red group-hover:text-accent-red">
                  <svg
              ...
              <div className="mt-7 flex flex-wrap items-center gap-2.5">
                ...
                <span className="inline-flex items-center gap-2 border border-border bg-background-secondary/80 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[0.18em] text-foreground-muted">
                  <span className="h-1.5 w-1.5 rounded-full bg-accent-gold" />
                  {divisionLabel}
                </span>
              </div>
              ...
              <h1 className="profile-reveal mt-5 max-w-4xl break-words font-display text-4xl font-black leading-[0.9] tracking-[-0.035em] text-foreground sm:text-5xl md:text-6xl xl:text-7xl">
                {wrestler.ringName}
              </h1>
              ...
              {wrestler.nickname && (
                <p className="profile-reveal mt-4 font-display text-base font-medium text-accent-gold sm:text-lg md:text-xl">
                  &ldquo;{wrestler.nickname}&rdquo;
                </p>
              )}
              ...
              {wrestler.members &&
                wrestler.members.length > 0 && (
                  <p className="profile-reveal mt-3 font-mono text-[9px] font-bold uppercase tracking-[0.18em] text-accent-gold sm:text-[10px]">
                    Team: {wrestler.members.join(" / ")}
                  </p>
                )}
              ...
              {wrestler.bio && (
                <p className="profile-reveal mt-5 max-w-2xl font-body text-sm leading-7 text-foreground-muted md:text-[15px]">
                  {wrestler.bio}
                </p>
              )}
              ...
              <div className="profile-reveal mt-7 grid grid-cols-2 gap-px overflow-hidden border border-border bg-border sm:grid-cols-4">
                <ProfileStat
                  label="HEIGHT"
              ...
              <div className="profile-reveal mt-7 flex flex-wrap gap-3">
                <Link
                  href="#match-history"
                  className="inline-flex items-center gap-2 border border-accent-red bg-accent-red/10 px-4 py-3 font-body text-[10px] font-bold uppercase tracking-[0.18em] text-accent-red transition-all duration-200 hover:bg-accent-red hover:text-background sm:px-5 sm:text-xs"
                >
                  VIEW MATCH HISTORY
                ...
                {wrestlerChampionships.length > 0 && (
                  <Link
                    href="#championships"
                    className="inline-flex items-center gap-2 border border-border bg-background-secondary/80 px-4 py-3 font-body text-[10px] font-bold uppercase tracking-[0.18em] text-foreground transition-all duration-200 hover:border-accent-gold hover:text-accent-gold sm:px-5 sm:text-xs"
                  >
                    VIEW TITLES
            ...
            <div className="profile-reveal lg:order-2">
              <div className="relative mx-auto w-full max-w-[280px] sm:max-w-[310px] lg:max-w-[340px] xl:max-w-[390px]">
                {/* Glow */}

                <div
                  className="profile-glow absolute -inset-4 rounded-[28px]"
                  aria-hidden="true"
                />

                {/* Frame */}

                <div className="relative overflow-hidden border border-white/[0.08] bg-background-secondary shadow-2xl shadow-black/50">
                  {/* Fixed image ratio */}
                  ...
                  <div className="relative aspect-[4/5] w-full">
                    {wrestler.image ? (
                      <Image
                        ...
                        alt={wrestler.ringName}
                        fill
                        sizes="(max-width: 640px) 280px, (max-width: 1024px) 310px, 390px"
                        className="object-cover object-top grayscale transition-all duration-700 hover:scale-[1.025] hover:grayscale-0"
                        priority
                      />
                    ) : (
                      <div className="flex h-full w-full flex-col items-center justify-center bg-background-secondary p-6 text-center">
                        <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full border border-white/10 bg-black/30">
                          <span className="font-display text-3xl font-black text-accent-red">
                            {wrestler.ringName?.charAt(0) || "W"}
                          </span>
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5">
                      <div className="flex items-end justify-between gap-4">
                        <div className="min-w-0">
                          <span className="block font-mono text-[9px] uppercase tracking-[0.2em] text-foreground-muted">
                            {roleLabel}
                          </span>

                          <span className="mt-1 block truncate font-display text-sm font-bold text-foreground sm:text-base">
                            {divisionLabel}
                          </span>
                        </div>

                        <div className="shrink-0 text-right">
                          <span className="block font-display text-2xl font-black leading-none text-accent-gold sm:text-3xl">
                            {winRateLabel}
                          </span>

                          <span className="mt-1 block font-mono text-[8px] uppercase tracking-[0.18em] text-foreground-muted">
                            win rate
                          </span>
                        </div>
                      </div>
                ...
                <div
                  className="absolute -right-2 top-8 h-20 w-px bg-accent-red/60"
                  aria-hidden="true"
                />

                <div
                  className="absolute -bottom-2 left-8 h-px w-20 bg-accent-gold/60"
                  aria-hidden="true"
                />
              </div>
            </div>
      ...
      >
        ...
        <div className="container-wide relative z-10 grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(320px,500px)]">
          <div>
            <span className="font-body text-[10px] font-bold uppercase tracking-[0.24em] text-accent-red sm:text-xs">
              PERFORMANCE INDEX
            </span>
        ...
        >
          <div className="container-wide">
            <SectionTitle
              ...
              accentColor="gold"
              right={
                <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-foreground-muted sm:text-xs">
                  {wrestlerChampionships.length}{" "}
                  {wrestlerChampionships.length === 1
                    ? "title"
                    : "titles"}{" "}
                  held
                </span>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              {wrestlerChampionships.map((championship) => (
                <ChampionshipCard
                  ...
                  championship={championship}
                  isCurrentChampion={
                    championship.currentChampion?.wrestlerId ===
                    wrestler.id
                  }
                />
              ))}
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {rivals.map((rival) => (
                ...
                >
                  ...
                  <div className="relative z-10 h-16 w-16 shrink-0 overflow-hidden border border-border bg-background-secondary">
                    {rival.image ? (
                      <Image
                        src={rival.image}
                        alt={rival.ringName}
                        fill
                        sizes="64px"
                        className="object-cover object-top grayscale transition-all duration-300 group-hover:scale-110 group-hover:grayscale-0"
                      />
                    ) : (
                      <div className="flex h-full w-full items-center justify-center p-2 text-center font-mono text-[8px] uppercase tracking-widest text-foreground-muted">
                        No image
                      </div>
                    )}
                  </div>
                  ...
                  <div className="relative z-10 min-w-0 flex-1">
                    <h3 className="truncate font-display text-sm font-bold text-foreground transition-colors group-hover:text-accent-red">
                      {rival.ringName}
                    </h3>
                    ...
                    <p className="mt-2 font-mono text-[9px] uppercase tracking-[0.14em] text-foreground-muted">
                      {rival.record.wins}W ·{" "}
                      {rival.record.losses}L ·{" "}
                      {rival.record.draws}D
                    </p>
                  </div>
  ...
  return (
    <div className="mb-10 flex flex-wrap items-end justify-between gap-5">
      <div>
        <span
          className={cn(
            "font-body text-[10px] font-bold uppercase tracking-[0.24em] sm:text-xs",
            overlineClass
          )}
        >
          {overline}
        ...
        <div
          className={cn(
            "mt-2 h-px w-12 bg-gradient-to-r to-transparent",
            ruleClass
          )}
        />
      </div>
...
function QuickStat({
  ...
  return (
    <div>
      <span
        className={cn(
          "block font-display text-2xl font-black",
          valueClass
        )}
      >
        {value}
...
function ProfileStat({
  ...
  return (
    <div
      className={cn(
        "group min-w-0 bg-background-secondary p-4 transition-colors duration-200 hover:bg-background-tertiary",
        className
      )}
    >
      <span className="block font-mono text-[8px] uppercase tracking-[0.18em] text-foreground-muted sm:text-[9px]">
        {label}
      </span>
...
function RecordPanel({
  ...
  const winPercent =
    total > 0
      ? (wrestler.record.wins / total) * 100
      : 0;

  const lossPercent =
    total > 0
      ? (wrestler.record.losses / total) * 100
      : 0;

  const drawPercent =
    total > 0
      ? (wrestler.record.draws / total) * 100
      : 0;
  ...
  return (
    <div className="relative overflow-hidden border border-border bg-background p-5 shadow-2xl shadow-black/20 sm:p-6 md:p-8">
      {/* Top line */}
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-foreground-muted sm:text-[10px]">
            Career record
          </span>
        ...
        <div className="text-right">
          <span className="font-display text-4xl font-black leading-none text-accent-gold sm:text-5xl">
            {winRateLabel}
          </span>

          <span className="mt-2 block font-mono text-[9px] uppercase tracking-[0.18em] text-foreground-muted">
            win rate
          </span>
        </div>
      </div>
      ...
      <div className="mt-6">
        <div className="flex items-center justify-between font-mono text-[9px] uppercase tracking-[0.16em] text-foreground-muted sm:text-[10px]">
          <span>Outcome distribution</span>
          <span>{winRateLabel} win</span>
        </div>

        <div
          className="mt-3 flex h-2 overflow-hidden rounded-full bg-foreground/10"
          aria-hidden="true"
        >
          <span
            className="h-full bg-accent-red transition-all duration-700"
        ...
        <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 font-mono text-[9px] uppercase tracking-[0.14em] text-foreground-muted">
          <span className="flex items-center gap-2">
...
function ChampionshipCard({
  ...
  return (
    <div className="group relative overflow-hidden border border-accent-gold/20 bg-background p-5 transition-all duration-300 hover:border-accent-gold/60 hover:bg-background-secondary hover:shadow-xl hover:shadow-black/20 sm:p-6">
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-accent-gold to-transparent" />

      <div className="absolute inset-0 bg-accent-gold/0 transition-colors duration-300 group-hover:bg-accent-gold/[0.02]" />

      <div className="relative z-10 flex items-start justify-between gap-4">
        <div className="relative h-14 w-36 shrink-0 overflow-hidden sm:h-16 sm:w-44">
          <Image
            src={championship.image}
            alt={championship.name}
            fill
      ...
      <div className="relative z-10 mt-5 grid grid-cols-3 gap-2 border-t border-border pt-5 font-mono">
        <div>
          <span className="block text-[8px] uppercase tracking-[0.16em] text-foreground-muted sm:text-[9px]">
            reign
          </span>
...
function MatchHistoryItem({
  ...
  const opponent = match.competitors.find(
    (competitor) =>
      competitor.wrestlerId !== wrestler.id
  );

  const isWinner =
    match.result.winner === wrestler.id;
  ...
  return (
    <div className="group grid gap-5 border border-border bg-background p-4 transition-all duration-300 hover:border-accent-red hover:bg-background-secondary hover:shadow-lg hover:shadow-black/20 sm:p-5 md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center">
      {/* Result */}

      <div className="flex items-center gap-4">
        <span
          className={cn(
            "grid h-10 w-10 place-items-center border font-display text-lg font-black sm:h-11 sm:w-11",
            isWinner
              ? "border-accent-gold/40 bg-accent-gold/10 text-accent-gold"
              : isDraw
                ? "border-border bg-foreground/5 text-foreground-muted"
        ...
        <div className="hidden min-w-[5.5rem] font-mono text-[9px] uppercase tracking-[0.14em] text-foreground-muted sm:block">
          <span>result</span>
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 font-display">
          ...
          <span className="text-[10px] font-bold text-accent-red sm:text-xs">
            VS
          </span>

          <span className="text-sm font-bold text-foreground-muted sm:text-lg">
            {opponent?.wrestler ||
              "Unknown opponent"}
          </span>
        </div>

        <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 font-mono text-[9px] uppercase tracking-[0.14em] text-foreground-muted sm:text-[10px]">
          <span>
            {event?.name || match.eventName}
          </span>
      ...
      <div className="flex items-center justify-between gap-4 border-t border-border pt-4 md:flex-col md:items-end md:border-l md:border-t-0 md:pl-5 md:pt-0">
        <div className="text-right">
          <span
            className={cn(
              "inline-flex min-w-[4.5rem] justify-center border px-2.5 py-1 font-mono text-[9px] font-bold uppercase tracking-[0.14em]",
              resultClasses
            )}
          >
            {resultLabel}
          ...
          <span className="mt-2 block font-mono text-[9px] uppercase tracking-[0.12em] text-foreground-muted">
            {!isDraw && (
              <>
                {match.result.method} ·{" "}
                {match.result.time}
